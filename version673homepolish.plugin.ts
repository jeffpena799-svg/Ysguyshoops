  ...
  return {
    ...
    transform(source,id){
      if(id.endsWith("/src/App.tsx")){
        ...
        code=code.replace(brandPattern,'<img className="brandMark" src="/ys-guys-brand-icon.svg" alt="Y\'s Guys official brushstroke Y" style={{objectFit:"contain"}}/>');
        return {code,map:null};
      }
      if(id.endsWith("/src/components/HomeDashboard.tsx")){
        let code=source
          .replaceAll('src="/ys-guys-logo.svg"','src="/ys-guys-brand-icon.svg"')
          .replaceAll('src="/ys-guys-official-logo.svg"','src="/ys-guys-brand-icon.svg"');
