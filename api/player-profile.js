...
export default async function handler(request, response) {
  ...
  if (typeof playerId !== "string" || typeof position !== "string" || !allowedPositions.has(position)) {
    return response.status(400).json({ error: "Choose a valid player position" });
  }
  try {
    const result = await sql.begin(async transaction => {
      ...
      const nextPlayers = players.map(player => player.id === playerId ? { ...player, position } : player);
    ...
    return response.status(500).json({ error: "Position is temporarily unavailable" });
  }
}
