...
type View = "home" | "attendance" | "community" | "games" | "players" | "profile" | "compare" | "leaders" | "more" | "records" | "awards" | "seasons" | "standings" | "calendar" | "rules" | "hof" | "timeline" | "voting" | "studio" | "commissioner";
...
type LeagueData = { players: Player[]; games: Game[]; awards: Award[]; seasons: typeof initialSeasons; news:NewsStory[]; runs:SundayRun[]; polls:LeaguePoll[] };
...
function exportData(data:LeagueData){
  const blob=new Blob([JSON.stringify({...data,exportedAt:new Date().toISOString(),version:"4.5"},null,2)],{type:"application/json"});
...
export default function App(){
  ...
  const [adminTab,setAdminTab]=useState<"runs"|"games"|"players"|"news"|"polls"|"awards"|"data">("runs");
  ...
  const saveAll=(next?:{players?:Player[];games?:Game[];awards?:Award[];seasons?:typeof initialSeasons;news?:NewsStory[];runs?:SundayRun[];polls?:LeaguePoll[]})=>{
    const p=next?.players??players,g=next?.games??games,a=next?.awards??awards,se=next?.seasons??seasons,n=next?.news??news,r=next?.runs??runs,po=next?.polls??polls;
    localStorage.setItem("yg-players",JSON.stringify(p));localStorage.setItem("yg-games",JSON.stringify(g));localStorage.setItem("yg-awards",JSON.stringify(a));localStorage.setItem("yg-seasons",JSON.stringify(se));localStorage.setItem("yg-news",JSON.stringify(n));localStorage.setItem("yg-runs",JSON.stringify(r));localStorage.setItem("yg-polls",JSON.stringify(po));
    const data={players:p,games:g,awards:a,seasons:se,news:n,runs:r,polls:po};
    if(sessionToken){
  ...
  const hydrateData=(data:LeagueData)=>{
    const nextNews=data.news??initialNews,nextRuns=data.runs??initialRuns,nextPolls=data.polls??initialPolls;
    setPlayers(data.players);setGames(data.games);setAwards(data.awards);setSeasons(data.seasons);setNews(nextNews);setRuns(nextRuns);setPolls(nextPolls);
    localStorage.setItem("yg-players",JSON.stringify(data.players));localStorage.setItem("yg-games",JSON.stringify(data.games));localStorage.setItem("yg-awards",JSON.stringify(data.awards));localStorage.setItem("yg-seasons",JSON.stringify(data.seasons));localStorage.setItem("yg-news",JSON.stringify(nextNews));localStorage.setItem("yg-runs",JSON.stringify(nextRuns));localStorage.setItem("yg-polls",JSON.stringify(nextPolls));
  };
  ...
  const resetData=()=>{
    ...
    replaceData({players:initialPlayers,games:initialGames,awards:initialAwards,seasons:initialSeasons,news:initialNews,runs:initialRuns,polls:initialPolls});
  };
  ...
  return <div className="app"><style>{styles}</style>
    <header className="topbar">
      <button className="brand" onClick={()=>go("home")}><span className="ball">YG</span><span><b>Y'S GUYS</b><small>League Universe · v4.5</small></span></button>
    ...
    <main>
      ...
      {view==="attendance" && <AttendanceCenter runs={orderedRuns} players={players} onSubmit={submitRsvp}/>}
      {view==="timeline" && <LeagueTimeline games={games} awards={awards} news={news} runs={runs}/>}
      {view==="voting" && <VotingCenter polls={polls} players={players} onVote={submitVote}/>}
      ...
      {view==="commissioner" && <Page eyebrow="COMMISSIONER MODE · v4.0" title="Run the entire league universe." subtitle="Publish games, profiles, photos and community stories for every visitor.">
        {!sessionToken?<CommissionerLogin onLogin={(token)=>{sessionStorage.setItem("yg-session",token);setSessionToken(token);setCloudStatus("saving");fetch("/api/league",{method:"PUT",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({data:{players,games,awards,seasons,news,runs,polls}})}).then(async response=>{if(!response.ok)throw new Error();const result=await response.json();setCloudUpdatedAt(result.updatedAt);setCloudStatus("cloud");setToast("Commissioner unlocked · league published");}).catch(()=>{setCloudStatus("error");setToast("Unlocked · first cloud save needs retry");}).finally(()=>setTimeout(()=>setToast(""),2400))}}/>:<>
        ...
        <div className="adminTabs">
          {([['runs','Sunday Runs'],['games','Games'],['players','Players'],['news','Community News'],['polls','Voting'],['awards','Awards'],['data','Backups']] as const).map(([k,l])=><button key={k} className={adminTab===k?'active':''} onClick={()=>setAdminTab(k)}>{l}</button>)}
        </div>
        {adminTab==='runs' && <RunManager runs={runs} onChange={(next)=>{setRuns(next);saveAll({runs:next});}} onConvert={(run)=>{if(games.some(game=>game.id===`game-${run.id}`))return alert("A scheduled game already exists for this Sunday.");const game:Game={id:`game-${run.id}`,date:formatRunDate(run.date),startTime:run.startTime,location:run.location,status:"scheduled",title:run.title,teamA:"Team A",scoreA:0,teamB:"Team B",scoreB:0,mvp:"",recap:""};const next=[...games,game];setGames(next);saveAll({games:next});setToast("Sunday added to scheduled games");setTimeout(()=>setToast(""),2000)}}/>}
        ...
        {adminTab==='data' && <DataTools data={{players,games,awards,seasons,news,runs,polls}} sessionToken={sessionToken} onImport={replaceData} onCloudRestore={(data,updatedAt)=>{hydrateData(data);setCloudUpdatedAt(updatedAt);setCloudStatus("cloud");setToast("Cloud revision restored");setTimeout(()=>setToast(""),2000)}} onReset={resetData}/>}
        </>}
      </Page>}

      {view==="more" && <Page eyebrow="LEAGUE UNIVERSE · v4.5" title="More from Y's Guys" subtitle="Every corner of the league in one connected universe."><div className="menuList"><Menu label="Sunday Attendance" icon="✅" onClick={()=>go("attendance")}/><Menu label="Community News" icon="📰" onClick={()=>go("community")}/><Menu label="League Timeline" icon="🗓️" onClick={()=>go("timeline")}/><Menu label="Voting Center" icon="🗳️" onClick={()=>go("voting")}/><Menu label="Share Studio" icon="🎨" onClick={()=>go("studio")}/><Menu label="Player Profiles" icon="🎮" onClick={()=>go("players")}/><Menu label="Compare Players" icon="⚔️" onClick={()=>go("compare")}/><Menu label="League Calendar" icon="📅" onClick={()=>go("calendar")}/><Menu label="Rule Book" icon="📖" onClick={()=>go("rules")}/><Menu label="Standings" icon="📊" onClick={()=>go("standings")}/><Menu label="Record Book" icon="🏆" onClick={()=>go("records")}/><Menu label="Awards Center" icon="🥇" onClick={()=>go("awards")}/><Menu label="Season Archive" icon="🗂️" onClick={()=>go("seasons")}/><Menu label="Hall of Fame & Legacy" icon="🏛️" onClick={()=>go("hof")}/><Menu label="Share League" icon="↗️" onClick={shareLeague}/><Menu label="Commissioner Mode" icon="🔒" onClick={()=>go("commissioner")}/></div></Page>}
    </main>

    {toast && <div className="toast">✓ {toast}</div>}
    <nav className="bottomNav">
      <Nav label="Home" icon="⌂" active={view==="home"} onClick={()=>go("home")}/><Nav label="Sunday" icon="✓" active={view==="attendance"} onClick={()=>go("attendance")}/><Nav label="Games" icon="◉" active={view==="games"} onClick={()=>go("games")}/><Nav label="Profiles" icon="◎" active={["players","profile"].includes(view)} onClick={()=>go("players")}/><Nav label="More" icon="•••" active={["more","community","timeline","voting","studio","leaders","compare","records","awards","seasons","standings","calendar","rules","hof","commissioner"].includes(view)} onClick={()=>go("more")}/>
    </nav>
...
function LeagueTimeline({games,awards,news,runs}:{games:Game[];awards:Award[];news:NewsStory[];runs:SundayRun[]}){
  ...
  return <Page eyebrow="LEAGUE HISTORY · v4.5" title="The Y's Guys timeline." subtitle="Games, awards, stories and Sunday runs preserved as one continuous history."><div className="chips">{["All","Games","Awards","News","Sundays"].map(value=><button className={filter===value?"active":""} onClick={()=>setFilter(value)} key={value}>{value}</button>)}</div><section className="timeline">{visible.map(item=><article className="timelineItem" key={item.id}><span className="timelineIcon">{item.icon}</span><div><small>{item.kind} · {item.date}</small><h2>{item.title}</h2><p>{item.copy}</p></div></article>)}</section></Page>;
}

function VotingCenter({polls,players,onVote}:{polls:LeaguePoll[];players:Player[];onVote:(pollId:string,voterId:string,nomineeId:string)=>Promise<void>}){
...
function AttendanceCenter({runs,players,onSubmit}:{runs:SundayRun[];players:Player[];onSubmit:(runId:string,rsvp:Omit<RunRsvp,"updatedAt">)=>Promise<void>}){
