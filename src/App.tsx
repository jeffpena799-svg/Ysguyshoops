...
type AdminTab = "dashboard"|"runs"|"games"|"players"|"history"|"news"|"polls"|"awards"|"data";
...
function exportData(data:LeagueData){
  const blob=new Blob([JSON.stringify({...data,exportedAt:new Date().toISOString(),version:"4.7"},null,2)],{type:"application/json"});
...
export default function App(){
  ...
  const hydrateData=(data:LeagueData)=>{
    const nextNews=data.news??initialNews,nextRuns=data.runs??initialRuns,nextPolls=data.polls??initialPolls,nextHistory=data.history??initialHistory,nextGames=normalizeLegacyGames(data.games);
    setPlayers(data.players);setGames(nextGames);setAwards(data.awards);setSeasons(data.seasons);setNews(nextNews);setRuns(nextRuns);setPolls(nextPolls);setHistory(nextHistory);
  ...
  return <div className="app"><style>{styles}</style>
    <header className="topbar">
      <button className="brand" onClick={()=>go("home")}><span className="ball">YG</span><span><b>Y'S GUYS</b><small>League Universe · v4.7</small></span></button>
    ...
    <main>
      ...
      {view==="commissioner" && <Page eyebrow="COMMISSIONER MODE · v4.0" title="Run the entire league universe." subtitle="Publish games, profiles, photos and community stories for every visitor.">
        ...
        <div className="adminTabs">
          {([['dashboard','Command Center'],['runs','Sunday Runs'],['games','Games'],['players','Add / Edit Players'],['history','Add History'],['news','Community News'],['polls','Voting'],['awards','Awards'],['data','Backups']] as const).map(([k,l])=><button key={k} className={adminTab===k?'active':''} onClick={()=>setAdminTab(k)}>{l}</button>)}
        </div>
