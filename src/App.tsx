...
type View = "home" | "community" | "games" | "players" | "profile" | "compare" | "leaders" | "more" | "records" | "awards" | "seasons" | "standings" | "calendar" | "rules" | "hof" | "commissioner";
...
type LeagueData = { players: Player[]; games: Game[]; awards: Award[]; seasons: typeof initialSeasons; news:NewsStory[] };
...
export default function App(){
  ...
  const [adminTab,setAdminTab]=useState<"games"|"players"|"news"|"awards"|"data">("games");
  ...
  const saveAll=(next?:{players?:Player[];games?:Game[];awards?:Award[];seasons?:typeof initialSeasons;news?:NewsStory[]})=>{
    const p=next?.players??players,g=next?.games??games,a=next?.awards??awards,se=next?.seasons??seasons,n=next?.news??news;
    localStorage.setItem("yg-players",JSON.stringify(p));localStorage.setItem("yg-games",JSON.stringify(g));localStorage.setItem("yg-awards",JSON.stringify(a));localStorage.setItem("yg-seasons",JSON.stringify(se));localStorage.setItem("yg-news",JSON.stringify(n));
    const data={players:p,games:g,awards:a,seasons:se,news:n};
    if(sessionToken){
  ...
  const hydrateData=(data:LeagueData)=>{
    const nextNews=data.news??initialNews;
    setPlayers(data.players);setGames(data.games);setAwards(data.awards);setSeasons(data.seasons);setNews(nextNews);
    localStorage.setItem("yg-players",JSON.stringify(data.players));localStorage.setItem("yg-games",JSON.stringify(data.games));localStorage.setItem("yg-awards",JSON.stringify(data.awards));localStorage.setItem("yg-seasons",JSON.stringify(data.seasons));localStorage.setItem("yg-news",JSON.stringify(nextNews));
  };
  ...
  const resetData=()=>{
    ...
    replaceData({players:initialPlayers,games:initialGames,awards:initialAwards,seasons:initialSeasons,news:initialNews});
  };
  ...
  return <div className="app"><style>{styles}</style>
    <header className="topbar">
      <button className="brand" onClick={()=>go("home")}><span className="ball">YG</span><span><b>Y'S GUYS</b><small>League Universe · v4.0</small></span></button>
    ...
    <main>
      ...
      {view==="commissioner" && <Page eyebrow="COMMISSIONER MODE · v4.0" title="Run the entire league universe." subtitle="Publish games, profiles, photos and community stories for every visitor.">
        {!sessionToken?<CommissionerLogin onLogin={(token)=>{sessionStorage.setItem("yg-session",token);setSessionToken(token);setCloudStatus("saving");fetch("/api/league",{method:"PUT",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({data:{players,games,awards,seasons,news}})}).then(async response=>{if(!response.ok)throw new Error();const result=await response.json();setCloudUpdatedAt(result.updatedAt);setCloudStatus("cloud");setToast("Commissioner unlocked · league published");}).catch(()=>{setCloudStatus("error");setToast("Unlocked · first cloud save needs retry");}).finally(()=>setTimeout(()=>setToast(""),2400))}}/>:<>
        ...
        <div className="adminTabs">
          {([['games','Games'],['players','Players'],['news','Community News'],['awards','Awards'],['data','Backups']] as const).map(([k,l])=><button key={k} className={adminTab===k?'active':''} onClick={()=>setAdminTab(k)}>{l}</button>)}
        </div>
        ...
        {adminTab==='data' && <DataTools data={{players,games,awards,seasons,news}} sessionToken={sessionToken} onImport={replaceData} onCloudRestore={(data,updatedAt)=>{hydrateData(data);setCloudUpdatedAt(updatedAt);setCloudStatus("cloud");setToast("Cloud revision restored");setTimeout(()=>setToast(""),2000)}} onReset={resetData}/>}
        </>}
      </Page>}

      {view==="more" && <Page eyebrow="LEAGUE UNIVERSE · v4.0" title="More from Y's Guys" subtitle="Every corner of the league in one connected universe."><div className="menuList"><Menu label="Community News" icon="📰" onClick={()=>go("community")}/><Menu label="Player Profiles" icon="🎮" onClick={()=>go("players")}/><Menu label="Compare Players" icon="⚔️" onClick={()=>go("compare")}/><Menu label="League Calendar" icon="📅" onClick={()=>go("calendar")}/><Menu label="Rule Book" icon="📖" onClick={()=>go("rules")}/><Menu label="Standings" icon="📊" onClick={()=>go("standings")}/><Menu label="Record Book" icon="🏆" onClick={()=>go("records")}/><Menu label="Awards Center" icon="🥇" onClick={()=>go("awards")}/><Menu label="Season Archive" icon="🗂️" onClick={()=>go("seasons")}/><Menu label="Hall of Fame & Legacy" icon="🏛️" onClick={()=>go("hof")}/><Menu label="Share League" icon="↗️" onClick={shareLeague}/><Menu label="Commissioner Mode" icon="🔒" onClick={()=>go("commissioner")}/></div></Page>}
    </main>

    {toast && <div className="toast">✓ {toast}</div>}
    <nav className="bottomNav">
      <Nav label="Home" icon="⌂" active={view==="home"} onClick={()=>go("home")}/><Nav label="Games" icon="◉" active={view==="games"} onClick={()=>go("games")}/><Nav label="Profiles" icon="◎" active={["players","profile"].includes(view)} onClick={()=>go("players")}/><Nav label="Community" icon="▤" active={view==="community"} onClick={()=>go("community")}/><Nav label="More" icon="•••" active={["more","leaders","compare","records","awards","seasons","standings","calendar","rules","hof","commissioner"].includes(view)} onClick={()=>go("more")}/>
    </nav>
