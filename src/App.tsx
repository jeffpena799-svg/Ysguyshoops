...
function exportData(data:LeagueData){
  const blob=new Blob([JSON.stringify({...data,exportedAt:new Date().toISOString(),version:"4.8.1"},null,2)],{type:"application/json"});
  ...
  return <div className="app"><style>{styles}</style>
    ...
    <main>
      ...
      {view==="players" && <Page eyebrow="PLAYER DIRECTORY" title="The people who built the league." subtitle="Search the roster and open any profile.">
        ...
        <div className="playerGrid">{filtered.map(p=><button className="playerCard" key={p.id} onClick={()=>openProfile(p)}>{p.photoUrl?<img className="playerThumb" src={p.photoUrl} alt={`${p.name} profile`}/>:<span className="bigAvatar">{overallRating(p)}</span>}<span className="pos">{p.jerseyNumber?`#${p.jerseyNumber} · `:""}{p.position}</span><h3>{p.name}</h3><small>“{p.nickname}” · {archetype(p)}</small><div className="miniStats"><span><b>{avg(p.pts,p)}</b>PPG</span><span><b>{avg(p.reb,p)}</b>RPG</span><span><b>{avg(p.ast,p)}</b>APG</span></div><div className="record">{p.wins}-{p.losses} · {hallProgress(hallResume(p,awards).total)}% Hall Progress</div></button>)}</div>
      </Page>}
      ...
      {view==="hof" && <Page eyebrow="HALL OF FAME · FORMULA V1" title="The road to immortality." subtitle="Only official awards, career milestones and wins count. Reach 100% Hall Progress to become eligible for induction.">
        {hallOfFame.length?<div className="hallGrid">{hallOfFame.map(p=>{const resume=hallResume(p,awards);return <button className="hallCard" key={p.id} onClick={()=>openProfile(p)}><span>🏛️</span><small>HALL OF FAME ELIGIBLE</small><h3>{p.name}</h3><p>{archetype(p)}</p><b>100% Hall Progress</b></button>})}</div>:<div className="hallEmpty"><span>🏛️</span><h2>The standard is set at 100%.</h2><p>No player is eligible yet. Every award, milestone and win now moves the percentage forward.</p></div>}
        <Section eyebrow="EVERY PLAYER" title="Hall Progress Tracker"/>
        <div className="legacyTracker">{players.slice().sort((a,b)=>hallResume(b,awards).total-hallResume(a,awards).total).map((p,index)=>{const resume=hallResume(p,awards),progress=hallProgress(resume.total);return <button key={p.id} onClick={()=>openProfile(p)}><span>{index+1}</span><div><b>{p.name}</b><small>{resume.status} · {progress}% complete</small><i><em style={{width:`${progress}%`}}/></i></div><strong>{progress}%</strong></button>})}</div>
      </Page>}
