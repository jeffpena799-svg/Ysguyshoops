...
type View = "home" | "games" | "players" | "profile" | "leaders" | "more" | "records" | "awards" | "seasons" | "standings" | "calendar" | "rules" | "hof" | "commissioner";
...
export default function App(){
  ...
  return <div className="app"><style>{styles}</style>
    <header className="topbar">
      <button className="brand" onClick={()=>go("home")}><span className="ball">YG</span><span><b>Y'S GUYS</b><small>League Universe · v2.9</small></span></button>
    ...
    <main>
      ...
      {view==="players" && <Page eyebrow="PLAYER DIRECTORY" title="The people who built the league." subtitle="Search the roster and open any profile.">
        ...
        <div className="playerGrid">{filtered.map(p=><button className="playerCard" key={p.id} onClick={()=>openProfile(p)}><span className="bigAvatar">{overallRating(p)}</span><span className="pos">{p.position}</span><h3>{p.name}</h3><small>“{p.nickname}” · {archetype(p)}</small><div className="miniStats"><span><b>{avg(p.pts,p)}</b>PPG</span><span><b>{avg(p.reb,p)}</b>RPG</span><span><b>{avg(p.ast,p)}</b>APG</span></div><div className="record">{p.wins}-{p.losses} · Legacy {legacyPercentage(p)}%</div></button>)}</div>
      </Page>}
      ...
      {view==="more" && <Page eyebrow="LEAGUE UNIVERSE · v2.9" title="More from Y's Guys" subtitle="Every corner of the league in one connected universe."><div className="menuList"><Menu label="Player Profiles" icon="🎮" onClick={()=>go("players")}/><Menu label="League Calendar" icon="📅" onClick={()=>go("calendar")}/><Menu label="Rule Book" icon="📖" onClick={()=>go("rules")}/><Menu label="Standings" icon="📊" onClick={()=>go("standings")}/><Menu label="Record Book" icon="🏆" onClick={()=>go("records")}/><Menu label="Awards Center" icon="🥇" onClick={()=>go("awards")}/><Menu label="Season Archive" icon="🗂️" onClick={()=>go("seasons")}/><Menu label="Hall of Fame" icon="🏛️" onClick={()=>go("hof")}/><Menu label="Share League" icon="↗️" onClick={shareLeague}/><Menu label="Commissioner Mode" icon="🔒" onClick={()=>go("commissioner")}/></div></Page>}
    </main>

    {toast && <div className="toast">✓ {toast}</div>}
    <nav className="bottomNav">
      <Nav label="Home" icon="⌂" active={view==="home"} onClick={()=>go("home")}/><Nav label="Games" icon="◉" active={view==="games"} onClick={()=>go("games")}/><Nav label="Profiles" icon="◎" active={["players","profile"].includes(view)} onClick={()=>go("players")}/><Nav label="Leaders" icon="↗" active={view==="leaders"} onClick={()=>go("leaders")}/><Nav label="More" icon="•••" active={["more","records","awards","seasons","standings","calendar","rules","hof","commissioner"].includes(view)} onClick={()=>go("more")}/>
    </nav>
...
function PlayerUniverseProfile({player,games,officialAwards,rank,onBack}:{player:Player;games:Game[];officialAwards:Award[];rank:number;onBack:()=>void}){
  ...
  return <><button className="backButton" onClick={onBack}>← All profiles</button><section className="universeHero"><div className="ratingOrb"><strong>{rating}</strong><small>OVR</small></div><div className="universeIdentity"><span>{player.position} · {archetype(player)}</span><h1>{player.name}</h1><p>“{player.nickname}”</p><div className="profileTags"><b>#{rank} OVERALL</b><b>{player.wins}-{player.losses} RECORD</b><b>{pct(player)}% WIN</b></div></div><div className="legacyMeter"><div><strong>{legacy}%</strong><small>LEGACY</small></div><span><i style={{width:`${legacy}%`}}/></span><p>{legacy>=80?"Hall of Fame lock":legacy>=60?"Hall of Fame trajectory":legacy>=40?"Building a strong résumé":"Legacy journey underway"}</p></div></section>
  ...
  <section className="profilePanel"><Section eyebrow="TROPHY CASE" title="Awards & honors"/>{honors.length?honors.map(honor=><div className="profileHonor" key={honor}><span>🏆</span><b>{honor}</b></div>):<div className="empty">No official honors recorded yet.</div>}</section></div>
