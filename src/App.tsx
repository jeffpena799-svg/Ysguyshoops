...
type Game = { id: string; date: string; title: string; teamA: string; scoreA: number; teamB: string; scoreB: number; mvp: string; recap: string; boxScore?:StatLine[] };
...
function applyBoxScoreDelta(players:Player[],oldGame:Game|undefined,newGame:Game|undefined){
  const apply=(list:Player[],game:Game|undefined,multiplier:number)=>{
    if(!game?.boxScore?.length)return list;
...
function teamStandings(games:Game[]){
  ...
  games.forEach(game=>{const a=row(game.teamA),b=row(game.teamB);a.pf+=game.scoreA;a.pa+=game.scoreB;b.pf+=game.scoreB;b.pa+=game.scoreA;if(game.scoreA>game.scoreB){a.wins++;b.losses++}else if(game.scoreB>game.scoreA){b.wins++;a.losses++}});
...
export default function App(){
  ...
  const records:RecordItem[]=useMemo(()=>[
    ...
    {category:"League",label:"Closest Game",holder:[...games].sort((a,b)=>Math.abs(a.scoreA-a.scoreB)-Math.abs(b.scoreA-b.scoreB))[0]?.title??"—",value:games.length?`${Math.min(...games.map(g=>Math.abs(g.scoreA-g.scoreB)))} points`:"—",date:"Calculated automatically"},
  ],[players,games,standings,pointsLeader,reboundsLeader,assistsLeader,winLeader]);
...
function GameManager({games,players,onSave,onDelete}:{games:Game[];players:Player[];onSave:(game:Game,previous?:Game)=>void;onDelete:(game:Game)=>void}){
  const empty:Game={id:"",date:"",title:"",teamA:"",scoreA:0,teamB:"",scoreB:0,mvp:players[0]?.name??"",recap:"",boxScore:[]};
