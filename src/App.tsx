...
type LeagueData = { players: Player[]; games: Game[]; awards: Award[]; seasons: typeof initialSeasons; news:NewsStory[]; runs:SundayRun[]; polls:LeaguePoll[]; history:HistoryEntry[] };
...
function exportData(data:LeagueData){
  const blob=new Blob([JSON.stringify({...data,exportedAt:new Date().toISOString(),version:"4.6"},null,2)],{type:"application/json"});
...
export default function App(){
  ...
  const [adminTab,setAdminTab]=useState<"runs"|"games"|"players"|"history"|"news"|"polls"|"awards"|"data">("runs");
  ...
  const saveAll=(next?:{players?:Player[];games?:Game[];awards?:Award[];seasons?:typeof initialSeasons;news?:NewsStory[];runs?:SundayRun[];polls?:LeaguePoll[];history?:HistoryEntry[]})=>{
    const p=next?.players??players,g=next?.games??games,a=next?.awards??awards,se=next?.seasons??seasons,n=next?.news??news,r=next?.runs??runs,po=next?.polls??polls,h=next?.history??history;
    ...
    const data={players:p,games:g,awards:a,seasons:se,news:n,runs:r,polls:po,history:h};
    if(sessionToken){
  ...
  const hydrateData=(data:LeagueData)=>{
    ...
    setPlayers(data.players);setGames(data.games);setAwards(data.awards);setSeasons(data.seasons);setNews(nextNews);setRuns(nextRuns);setPolls(nextPolls);setHistory(nextHistory);
  ...
  const resetData=()=>{
    ...
    replaceData({players:initialPlayers,games:initialGames,awards:initialAwards,seasons:initialSeasons,news:initialNews,runs:initialRuns,polls:initialPolls,history:initialHistory});
  };
  ...
  return <div className="app"><style>{styles}</style>
    <header className="topbar">
      <button className="brand" onClick={()=>go("home")}><span className="ball">YG</span><span><b>Y'S GUYS</b><small>League Universe · v4.6</small></span></button>
    ...
    <main>
      ...
      {view==="commissioner" && <Page eyebrow="COMMISSIONER MODE · v4.0" title="Run the entire league universe." subtitle="Publish games, profiles, photos and community stories for every visitor.">
        {!sessionToken?<CommissionerLogin onLogin={(token)=>{sessionStorage.setItem("yg-session",token);setSessionToken(token);setCloudStatus("saving");fetch("/api/league",{method:"PUT",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({data:{players,games,awards,seasons,news,runs,polls,history}})}).then(async response=>{if(!response.ok)throw new Error();const result=await response.json();setCloudUpdatedAt(result.updatedAt);setCloudStatus("cloud");setToast("Commissioner unlocked · league published");}).catch(()=>{setCloudStatus("error");setToast("Unlocked · first cloud save needs retry");}).finally(()=>setTimeout(()=>setToast(""),2400))}}/>:<>
        ...
        <div className="adminTabs">
          {([['runs','Sunday Runs'],['games','Games'],['players','Add / Edit Players'],['history','Add History'],['news','Community News'],['polls','Voting'],['awards','Awards'],['data','Backups']] as const).map(([k,l])=><button key={k} className={adminTab===k?'active':''} onClick={()=>setAdminTab(k)}>{l}</button>)}
        </div>
        ...
        {adminTab==='data' && <DataTools data={{players,games,awards,seasons,news,runs,polls,history}} sessionToken={sessionToken} onImport={replaceData} onCloudRestore={(data,updatedAt)=>{hydrateData(data);setCloudUpdatedAt(updatedAt);setCloudStatus("cloud");setToast("Cloud revision restored");setTimeout(()=>setToast(""),2000)}} onReset={resetData}/>}
        </>}
      </Page>}
