...
type AdminTab = "dashboard"|"runs"|"games"|"players"|"history"|"news"|"polls"|"awards"|"branding"|"data";
...
type LeagueData = { players: Player[]; games: Game[]; awards: Award[]; seasons: typeof initialSeasons; news:NewsStory[]; runs:SundayRun[]; polls:LeaguePoll[]; history:HistoryEntry[]; branding:LeagueBranding };
...
function exportData(data:LeagueData){
  const blob=new Blob([JSON.stringify({...data,exportedAt:new Date().toISOString(),version:"4.8.0"},null,2)],{type:"application/json"});
...
export default function App(){
  ...
  const saveAll=(next?:{players?:Player[];games?:Game[];awards?:Award[];seasons?:typeof initialSeasons;news?:NewsStory[];runs?:SundayRun[];polls?:LeaguePoll[];history?:HistoryEntry[];branding?:LeagueBranding})=>{
    const p=next?.players??players,g=next?.games??games,a=next?.awards??awards,se=next?.seasons??seasons,n=next?.news??news,r=next?.runs??runs,po=next?.polls??polls,h=next?.history??history,b=next?.branding??branding;
    ...
    const data={players:p,games:g,awards:a,seasons:se,news:n,runs:r,polls:po,history:h,branding:b};
    if(sessionToken){
  ...
  const hydrateData=(data:LeagueData)=>{
    const nextNews=data.news??initialNews,nextRuns=data.runs??initialRuns,nextPolls=data.polls??initialPolls,nextHistory=data.history??initialHistory,nextBranding=data.branding??initialBranding,nextGames=normalizeLegacyGames(data.games);
    setPlayers(data.players);setGames(nextGames);setAwards(data.awards);setSeasons(data.seasons);setNews(nextNews);setRuns(nextRuns);setPolls(nextPolls);setHistory(nextHistory);setBranding(nextBranding);
  ...
  const resetData=()=>{
    ...
    replaceData({players:initialPlayers,games:initialGames,awards:initialAwards,seasons:initialSeasons,news:initialNews,runs:initialRuns,polls:initialPolls,history:initialHistory,branding:initialBranding});
  };
  ...
  return <div className="app"><style>{styles}</style>
    <header className="topbar">
      <button className="brand" onClick={()=>go("home")}><img className="brandMark" src={branding.logoUrl||initialBranding.logoUrl} alt=""/><span><b>{branding.wordmark}</b><small>{branding.tagline} · v4.8</small></span></button>
      <div className="headerActions"><button className="myPlayerPill" onClick={()=>setShowMyPlayerPicker(true)}>{myPlayer?<><span className="avatar">{initials(myPlayer.name)}</span><b>{myPlayer.name}</b></>:<>◎ <b>My Player</b></>}</button><button className="seasonPill" onClick={()=>go("seasons")}><span className={`syncDot ${cloudStatus}`}/>{cloudStatus==="cloud"?"Shared":"Offline"} · Summer 2026</button></div>
    </header>
    ...
    <main>
      {view==="home" && <>
        ...
        {nextRun&&<button className="nextRunBanner" onClick={()=>go("attendance")}><div><span>NEXT SUNDAY RUN</span><h2>{formatRunDate(nextRun.date)}</h2><p>{nextRun.startTime} · {nextRun.location}</p></div><div className="runCount"><strong>{runCounts(nextRun).going}</strong><small>GOING</small></div><div className="runCount maybe"><strong>{runCounts(nextRun).maybe}</strong><small>MAYBE</small></div><b>RSVP →</b></button>}
        {myPlayer&&<button className="myPlayerHome" onClick={()=>openProfile(myPlayer)}>{myPlayer.photoUrl?<img src={myPlayer.photoUrl} alt=""/>:<span>{overallRating(myPlayer)}</span>}<div><small>MY PLAYER</small><h2>{myPlayer.name}</h2><p>{myPlayer.wins}-{myPlayer.losses} · {avg(myPlayer.pts,myPlayer)} PPG · {hallResume(myPlayer,awards).total} Hall Points</p></div><b>Open profile →</b></button>}
        ...
        <div className="exploreGrid">
          ...
          <Explore icon="📰" title="Community" copy="League stories, photos and announcements." onClick={()=>go("community")}/>
          <Explore icon="🗓️" title="League Timeline" copy="Every game, award and milestone in order." onClick={()=>go("timeline")}/>
          <Explore icon="🗳️" title="Voting Center" copy="Cast a vote in active league polls." onClick={()=>go("voting")}/>
        </div>
      </>}
      ...
      {view==="studio" && <ShareStudio players={players} games={games} runs={runs} news={news}/>}
      ...
      {view==="players" && <Page eyebrow="PLAYER DIRECTORY" title="The people who built the league." subtitle="Search the roster and open any profile.">
        ...
        <div className="playerGrid">{filtered.map(p=><button className="playerCard" key={p.id} onClick={()=>openProfile(p)}>{p.photoUrl?<img className="playerThumb" src={p.photoUrl} alt={`${p.name} profile`}/>:<span className="bigAvatar">{overallRating(p)}</span>}<span className="pos">{p.jerseyNumber?`#${p.jerseyNumber} · `:""}{p.position}</span><h3>{p.name}</h3><small>“{p.nickname}” · {archetype(p)}</small><div className="miniStats"><span><b>{avg(p.pts,p)}</b>PPG</span><span><b>{avg(p.reb,p)}</b>RPG</span><span><b>{avg(p.ast,p)}</b>APG</span></div><div className="record">{p.wins}-{p.losses} · {hallResume(p,awards).total} Hall Points</div></button>)}</div>
      </Page>}
      ...
      {view==="hof" && <Page eyebrow="HALL OF FAME · FORMULA V1" title="The road to immortality." subtitle="Only official awards, career milestones and wins count. Reach 100 Hall Points to become eligible for induction.">
        {hallOfFame.length?<div className="hallGrid">{hallOfFame.map(p=>{const resume=hallResume(p,awards);return <button className="hallCard" key={p.id} onClick={()=>openProfile(p)}><span>🏛️</span><small>HALL OF FAME ELIGIBLE</small><h3>{p.name}</h3><p>{archetype(p)}</p><b>{resume.total} Hall Points</b></button>})}</div>:<div className="hallEmpty"><span>🏛️</span><h2>The standard is set at 100.</h2><p>No player is eligible yet. Every award, milestone and win now moves the race forward.</p></div>}
        <Section eyebrow="EVERY PLAYER" title="Hall Points Tracker"/>
        <div className="legacyTracker">{players.slice().sort((a,b)=>hallResume(b,awards).total-hallResume(a,awards).total).map((p,index)=>{const resume=hallResume(p,awards);return <button key={p.id} onClick={()=>openProfile(p)}><span>{index+1}</span><div><b>{p.name}</b><small>{resume.status} · {resume.awardPoints} award points</small><i><em style={{width:`${Math.min(100,resume.total)}%`}}/></i></div><strong>{resume.total}</strong></button>})}</div>
      </Page>}
      ...
      {view==="commissioner" && <Page eyebrow="COMMISSIONER MODE · v4.0" title="Run the entire league universe." subtitle="Publish games, profiles, photos and community stories for every visitor.">
        {!sessionToken?<CommissionerLogin onLogin={(token)=>{sessionStorage.setItem("yg-session",token);setSessionToken(token);setCloudStatus("saving");fetch("/api/league",{method:"PUT",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({data:{players,games,awards,seasons,news,runs,polls,history,branding}})}).then(async response=>{if(!response.ok)throw new Error();const result=await response.json();setCloudUpdatedAt(result.updatedAt);setCloudStatus("cloud");setToast("Commissioner unlocked · league published");}).catch(()=>{setCloudStatus("error");setToast("Unlocked · first cloud save needs retry");}).finally(()=>setTimeout(()=>setToast(""),2400))}}/>:<>
        ...
        <div className="adminTabs">
          {([['dashboard','Command Center'],['runs','Sunday Runs'],['games','Games'],['players','Add / Edit Players'],['history','Add History'],['news','Community News'],['polls','Voting'],['awards','Awards'],['branding','League Branding'],['data','Backups']] as const).map(([k,l])=><button key={k} className={adminTab===k?'active':''} onClick={()=>setAdminTab(k)}>{l}</button>)}
        </div>
        ...
        {adminTab==='data' && <DataTools data={{players,games,awards,seasons,news,runs,polls,history,branding}} sessionToken={sessionToken} onImport={replaceData} onCloudRestore={(data,updatedAt)=>{hydrateData(data);setCloudUpdatedAt(updatedAt);setCloudStatus("cloud");setToast("Cloud revision restored");setTimeout(()=>setToast(""),2000)}} onReset={resetData}/>}
        </>}
      </Page>}

      {view==="more" && <Page eyebrow="LEAGUE UNIVERSE · v4.8" title="More from Y's Guys" subtitle="A player-centered archive of every run, record and story."><div className="menuList"><Menu label="Community News" icon="📰" onClick={()=>go("community")}/><Menu label="League Timeline" icon="🗓️" onClick={()=>go("timeline")}/><Menu label="Voting Center" icon="🗳️" onClick={()=>go("voting")}/><Menu label="Share Studio" icon="🎨" onClick={()=>go("studio")}/><Menu label="Compare Players" icon="⚔️" onClick={()=>go("compare")}/><Menu label="League Calendar" icon="📅" onClick={()=>go("calendar")}/><Menu label="Rule Book" icon="📖" onClick={()=>go("rules")}/><Menu label="Record Book" icon="🏆" onClick={()=>go("records")}/><Menu label="Awards Center" icon="🥇" onClick={()=>go("awards")}/><Menu label="Season Archive" icon="🗂️" onClick={()=>go("seasons")}/><Menu label="Hall of Fame & Hall Points" icon="🏛️" onClick={()=>go("hof")}/><Menu label="Share League" icon="↗️" onClick={shareLeague}/><Menu label="Commissioner Mode" icon="🔒" onClick={()=>go("commissioner")}/></div></Page>}
    </main>
    ...
    <nav className="bottomNav">
      <Nav label="Home" icon="⌂" active={view==="home"} onClick={()=>go("home")}/><Nav label="Sunday" icon="✓" active={view==="attendance"} onClick={()=>go("attendance")}/><Nav label="Games" icon="◉" active={view==="games"} onClick={()=>go("games")}/><Nav label="Profiles" icon="◎" active={["players","profile"].includes(view)} onClick={()=>go("players")}/><Nav label="More" icon="•••" active={["more","community","timeline","voting","studio","leaders","compare","records","awards","seasons","calendar","rules","hof","commissioner"].includes(view)} onClick={()=>go("more")}/>
    </nav>
...
function PlayerUniverseProfile({player,games,officialAwards,rank,onBack}:{player:Player;games:Game[];officialAwards:Award[];rank:number;onBack:()=>void}){
  ...
  const rating=overallRating(player),resume=hallResume(player,officialAwards);
  return <><button className="backButton" onClick={onBack}>← All profiles</button><section className="universeHero" style={player.bannerColor?{background:`linear-gradient(135deg,#071c3e,${player.bannerColor})`}:undefined}>{player.photoUrl?<div className="profilePhotoWrap"><img src={player.photoUrl} alt={`${player.name} profile`}/><b>{rating} OVR</b></div>:<div className="ratingOrb"><strong>{rating}</strong><small>OVR</small></div>}<div className="universeIdentity"><span>{player.jerseyNumber?`#${player.jerseyNumber} · `:""}{player.position}{player.height?` · ${player.height}`:""} · {archetype(player)}</span><h1>{player.name}</h1><p>“{player.nickname}”</p><div className="profileTags"><b>#{rank} OVERALL</b><b>{player.wins}-{player.losses} RECORD</b><b>{pct(player)}% WIN</b>{player.overallOverride!==undefined&&<b>COMMISSIONER OVR</b>}{player.signatureBadge&&<b>⭐ {player.signatureBadge}</b>}</div></div><div className="legacyMeter"><div><strong>{resume.total}</strong><small>HALL POINTS</small></div><span><i style={{width:`${Math.min(100,resume.total)}%`}}/></span><p>{resume.status}{resume.total<100?` · ${100-resume.total} points to eligibility`:" · Eligible for induction"}</p></div></section>
...
function PlayerComparison({players,awards}:{players:Player[];awards:Award[]}){
  ...
  const rows=[
    ...
    ["Hall Points",hallResume(left,awards).total,hallResume(right,awards).total],
...
function VotingCenter({polls,players,defaultPlayerId,onVote}:{polls:LeaguePoll[];players:Player[];defaultPlayerId:string;onVote:(pollId:string,voterId:string,nomineeId:string)=>Promise<void>}){
  ...
  return <Page eyebrow="THE BALLOT · v4.5" title="Voting Center" subtitle="Choose your name, make your selections and help shape league history. No PIN required."><label className="voterSelect">Your name<select value={voterId} onChange={event=>{const id=event.target.value;setVoterId(id);const existing:Record<string,string>={};polls.forEach(poll=>{const vote=poll.votes.find(item=>item.playerId===id);if(vote)existing[poll.id]=vote.nomineeId});setChoices(existing)}}><option value="">Choose your name</option>{players.map(player=><option value={player.id} key={player.id}>{player.name}</option>)}</select></label>{error&&<div className="formError">{error}</div>}<div className="pollGrid">{polls.map(poll=>{const total=poll.votes.length,closed=poll.status==="closed"||Boolean(poll.deadline&&new Date(poll.deadline).getTime()<Date.now());return <article className="pollCard" key={poll.id}><header><span>{poll.category}</span><b>{closed?"CLOSED":"OPEN"}</b></header><h2>{poll.title}</h2><p>{poll.description}</p><div className="nomineeList">{poll.nomineeIds.map(id=>{const player=players.find(item=>item.id===id),votes=poll.votes.filter(item=>item.nomineeId===id).length,percent=total?Math.round(votes/total*100):0;return <button disabled={closed||!voterId} className={choices[poll.id]===id?"selected":""} onClick={()=>setChoices({...choices,[poll.id]:id})} key={id}><span className="avatar">{player?initials(player.name):"?"}</span><div><b>{player?.name??"Former player"}</b>{closed&&<i><em style={{width:`${percent}%`}}/></i>}</div>{closed?<strong>{votes} · {percent}%</strong>:<small>{choices[poll.id]===id?"Selected":"Choose"}</small>}</button>})}</div>{!closed&&<button className="primary voteButton" disabled={!voterId||!choices[poll.id]||saving===poll.id} onClick={()=>vote(poll)}>{saving===poll.id?"Saving…":poll.votes.some(item=>item.playerId===voterId)?"Update vote":"Submit vote"}</button>}<footer>{total} recorded {total===1?"vote":"votes"}{poll.deadline?` · Deadline ${new Date(poll.deadline).toLocaleString()}`:""}</footer></article>})}</div></Page>;
}
