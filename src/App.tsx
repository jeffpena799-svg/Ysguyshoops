...
function exportData(data:LeagueData){
  const blob=new Blob([JSON.stringify({...data,exportedAt:new Date().toISOString(),version:"6.0.0"},null,2)],{type:"application/json"});
...
export default function App(){
  ...
  return <div className="app"><style>{styles}</style>
    <header className="topbar">
      <button className="brand" onClick={()=>go("home")}><img className="brandMark" src={branding.logoUrl||initialBranding.logoUrl} alt=""/><span><b>{branding.wordmark}</b><small>{branding.tagline} · v6.0</small></span></button>
    ...
    <main>
      ...
      {view==="community" && <Page eyebrow="THE COMMUNITY · v6.0" title="More than a box score." subtitle="Stories, announcements and moments from around the Y's Guys universe.">
      ...
      {view==="games" && <Page eyebrow="GAME DAY · v6.0" title="Schedule and results." subtitle="Upcoming matchups and complete results from the active season.">
      ...
      {view==="profile" && selected && <PlayerUniverseProfile player={selected} roster={players} games={games} sundaySessions={sundaySessions} nextRun={nextRun} updatedAt={cloudUpdatedAt} officialAwards={awards} rank={players.slice().sort((a,b)=>(overallRating(b,players)??0)-(overallRating(a,players)??0)).findIndex(p=>p.id===selected.id)+1} isMyPlayer={selected.id===myPlayerId} onRsvp={submitRsvp} onPhotoSubmit={submitLeagueItem} onPositionChange={updateOwnPosition} onBack={()=>go("players")}/>}
      ...
      {view==="records" && <Page eyebrow="THE ARCHIVE" title="Y's Guys Record Book" subtitle="The marks everyone is chasing.">
        ...
        <div className="note">Version 6.0 recalculates this record book whenever shared player or game data changes.</div>
      </Page>}
      ...
      {view==="commissioner" && <Page eyebrow="COMMISSIONER MODE · v6.0" title="Run the entire league universe." subtitle="Publish games, profiles, photos and community stories for every visitor.">
...
function PlayerUniverseProfile({player,roster,games,sundaySessions,nextRun,updatedAt,officialAwards,rank,isMyPlayer,onRsvp,onPhotoSubmit,onPositionChange,onBack}:{player:Player;roster:Player[];games:Game[];sundaySessions:SundaySession[];nextRun?:SundayRun;updatedAt:string|null;officialAwards:Award[];rank:number;isMyPlayer:boolean;onRsvp:(runId:string,rsvp:Omit<RunRsvp,"updatedAt">)=>Promise<void>;onPhotoSubmit:(item:{type:"profile-photo"|"suggestion";playerId:string;message:string;imageUrl?:string})=>Promise<void>;onPositionChange:(playerId:string,position:string)=>Promise<void>;onBack:()=>void}){
  ...
  {isMyPlayer&&nextRun&&<QuickRsvp run={nextRun} myPlayer={player} updatedAt={updatedAt} onChoosePlayer={()=>{}} onSubmit={onRsvp}/>}
...
function MyPlayerPerformance({sessions}:{sessions:{session:SundaySession;line:SundayStatLine}[]}){
  const categories=[["Points","pts"],["Rebounds","reb"],["Assists","ast"],["Wins","wins"],["Games Played","gp"]] as const;
  const highs=categories.map(([label,key])=>{
    const max=sessions.reduce((best,item)=>Math.max(best,item.line[key]),0);
    return {label,max,dates:sessions.filter(item=>item.line[key]===max&&max>0).map(item=>item.session.date)};
  });
  return <section className="myPlayerPerformance">
    <Section eyebrow="MY PLAYER" title="Single-day highs"/>
...
function SuggestionBox({players,defaultPlayerId,onSubmit}:{players:Player[];defaultPlayerId:string;onSubmit:(item:{type:"profile-photo"|"suggestion";playerId:string;message:string;imageUrl?:string})=>Promise<void>}){
  ...
  return <Page eyebrow="PRIVATE INBOX · v6.0" title="Suggestion Box" subtitle="Send an idea, correction or picture directly to the Commissioner. Nothing appears publicly unless the Commissioner chooses to use it."><section className="suggestionCard">{done&&<div className="successNote">✓ Your suggestion is in the Commissioner Review Center.</div>}<label>Your player profile<select value={playerId} onChange={event=>setPlayerId(event.target.value)}>{players.map(player=><option value={player.id} key={player.id}>{player.name}</option>)}</select></label><label>Suggestion, idea or correction<textarea value={message} onChange={event=>setMessage(event.target.value)} maxLength={1200} placeholder="What should be added, corrected or remembered?"/></label><label className="uploadButton">Add one optional picture<input type="file" accept="image/*" onChange={event=>choose(event.target.files?.[0])}/></label>{imageUrl&&<div className="suggestionPreview"><img src={imageUrl} alt="Suggestion preview"/><button onClick={()=>setImageUrl("")}>Remove</button></div>}<p className="privacyNote">Pictures are compressed before upload. Video is not accepted in Version 6.0.</p><button className="primary" disabled={busy} onClick={send}>{busy?"Sending…":"Send privately"}</button>{error&&<p className="formError">{error}</p>}</section></Page>;
}
...
function LeagueTimeline({games,awards,news,runs,history}:{games:Game[];awards:Award[];news:NewsStory[];runs:SundayRun[];history:HistoryEntry[]}){
  ...
  return <Page eyebrow="LEAGUE HISTORY · v6.0" title="The Y's Guys timeline." subtitle="Games, awards, stories, Sundays and Commissioner-recorded memories preserved together."><div className="chips">{["All","History","Games","Awards","News","Sundays"].map(value=><button className={filter===value?"active":""} onClick={()=>setFilter(value)} key={value}>{value}</button>)}</div><section className="timeline">{visible.map(item=><article className={`timelineItem ${item.imageUrl?"withPhoto":""}`} key={item.id}>{item.imageUrl&&<img src={item.imageUrl} alt=""/>}<span className="timelineIcon">{item.icon}</span><div><small>{item.kind} · {item.date}</small><h2>{item.title}</h2><p>{item.copy}</p></div></article>)}</section></Page>;
}
...
function AttendanceCenter({runs,players,defaultPlayerId,onSubmit}:{runs:SundayRun[];players:Player[];defaultPlayerId:string;onSubmit:(runId:string,rsvp:Omit<RunRsvp,"updatedAt">)=>Promise<void>}){
  ...
  return <Page eyebrow="SUNDAY RUN · v6.0" title="Are you in?" subtitle="Choose your name and let the group know your Sunday availability. No account or PIN required.">
...
function RuleBook(){
  ...
  return <Page eyebrow="OFFICIAL RULE BOOK · v6.0" title="How the Y's Guys universe operates." subtitle="The official rules of the Sunday run."><div className="ruleHero"><span>YG</span><div><b>LEAGUE STANDARD</b><h2>Have fun win the day!</h2></div></div><div className="rulesGrid">{sections.map((section,index)=><section className="ruleSection" key={section.title}><div><span>{section.icon}</span><small>ARTICLE {index+1}</small></div><h2>{section.title}</h2><ol start={index+1}>{section.rules.map(rule=><li key={rule}>{rule}</li>)}</ol></section>)}</div></Page>;
}
