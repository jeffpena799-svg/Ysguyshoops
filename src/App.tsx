...
type SundaySession = {
  id:string; date:string; notes:string; status:"published"; lines:SundayStatLine[];
  sourceImageUrl?:string; createdAt:string; updatedAt:string;
};
...
const initialPlayers: Player[] = [
  ...
  { id:"vic", name:"Vic", nickname:"Henny Vic", position:"G", wins:12, losses:5, pts:73, reb:34, ast:17, turnovers:16, awards:["2025 Most Improved"], weeklyMvpCredits:[{id:"weekly-mvp-2025-history",season:"2025",count:6}], bio:"Aggressive scorer who can change the pace of a game in a hurry." },
  { id:"paul", name:"Paul Peters", nickname:"MVP", position:"F", wins:17, losses:4, pts:56, reb:39, ast:21, turnovers:7, awards:["2025 MVP"], weeklyMvpCredits:[{id:"weekly-mvp-2025-history",season:"2025",count:3}], bio:"Winning, versatility and consistency define the league's reigning MVP." },
  { id:"jose", name:"Jose", nickname:"Jose Jose Jose", position:"G", wins:7, losses:8, pts:61, reb:28, ast:8, turnovers:2, awards:[], weeklyMvpCredits:[{id:"weekly-mvp-2025-history",season:"2025",count:5}], bio:"A fearless scorer with a knack for timely buckets." },
  { id:"jeffrey", name:"Jeffrey", nickname:"Jeff", position:"SF", wins:6, losses:9, pts:58, reb:62, ast:12, turnovers:15, awards:[], weeklyMvpCredits:[{id:"weekly-mvp-2025-history",season:"2025",count:3}], bio:"Physical wing who can defend across positions and create inside." },
  { id:"ty", name:"Ty", nickname:"Ty Bry", position:"G", wins:13, losses:5, pts:45, reb:41, ast:23, turnovers:2, awards:[], weeklyMvpCredits:[{id:"weekly-mvp-2025-history",season:"2025",count:1}], bio:"Efficient connector who impacts winning with passing and defense." },
  ...
  { id:"alex", name:"Alex", nickname:"Big Al", position:"F", wins:5, losses:10, pts:53, reb:32, ast:19, turnovers:8, awards:["2025 Caruso Hustle Award"], weeklyMvpCredits:[{id:"weekly-mvp-2025-history",season:"2025",count:5}], bio:"High-energy forward who brings effort, passing and toughness." },
  { id:"nick-d", name:"Nick D", nickname:"Nick", position:"G", wins:5, losses:10, pts:32, reb:29, ast:19, turnovers:4, awards:["2025 Defensive Player of the Year"], weeklyMvpCredits:[{id:"weekly-mvp-2025-history",season:"2025",count:1}], bio:"Point-of-attack defender and steady secondary creator." },
  { id:"hunter", name:"Hunter", nickname:"Hunter Guy", position:"G", wins:3, losses:5, pts:18, reb:16, ast:9, turnovers:1, awards:[], weeklyMvpCredits:[{id:"weekly-mvp-2025-history",season:"2025",count:2}], bio:"Young guard with a balanced game and calm decision-making." },
  { id:"mario", name:"Mario", nickname:"Mario", position:"F", wins:6, losses:8, pts:24, reb:36, ast:17, turnovers:8, awards:[], weeklyMvpCredits:[{id:"weekly-mvp-2025-history",season:"2025",count:3}], bio:"Versatile frontcourt player who rebounds and keeps the ball moving." },
...
const initialSundaySessions:SundaySession[] = [];
...
type LeagueData = { players: Player[]; games: Game[]; awards: Award[]; seasons: typeof initialSeasons; news:NewsStory[]; runs:SundayRun[]; polls:LeaguePoll[]; history:HistoryEntry[]; branding:LeagueBranding; rankings:PowerRankingSnapshot[]; submissions:LeagueSubmission[]; sundaySessions:SundaySession[]; statBaseline:PlayerStatBaseline };
...
function syncWeeklyMvpPollCredits(players:Player[],polls:LeaguePoll[]){
  const clean=players.map(player=>({...player,weeklyMvpCredits:(player.weeklyMvpCredits??[]).filter(credit=>!credit.sourcePollId)}));
  return polls.filter(poll=>poll.category==="Weekly Award"&&poll.status==="closed"&&poll.officialWinnerId).reduce((roster,poll)=>roster.map(player=>player.id===poll.officialWinnerId?{...player,weeklyMvpCredits:[...(player.weeklyMvpCredits??[]),{id:`weekly-mvp-poll-${poll.id}`,season:new Date(poll.finalizedAt||poll.deadline||poll.createdAt).getFullYear().toString(),count:1,sourcePollId:poll.id,awardedAt:poll.finalizedAt||new Date().toISOString()}]}:player),clean);
}
function makeStatBaseline(players:Player[]):PlayerStatBaseline{
...
function applySundayLedger(current:Player[],baseline:PlayerStatBaseline,sessions:SundaySession[],games:Game[]=[]){
  const rebuilt=current.map(player=>{
    ...
    const lines=sessions.flatMap(session=>session.lines.filter(line=>line.playerId===player.id));
    return {...player,
      wins:base.wins+lines.reduce((sum,line)=>sum+line.wins,0),
      losses:base.losses+lines.reduce((sum,line)=>sum+Math.max(0,line.gp-line.wins),0),
      pts:base.pts+lines.reduce((sum,line)=>sum+line.pts,0),
      reb:base.reb+lines.reduce((sum,line)=>sum+line.reb,0),
      ast:base.ast+lines.reduce((sum,line)=>sum+line.ast,0),
      turnovers:base.turnovers+lines.reduce((sum,line)=>sum+line.turnovers,0),
    };
  });
  return applyOverallMovement(current,rebuilt,games);
}
function hallResume(player:Player,awards:Award[]){
  ...
  const weeklyMvpPoints=Math.round(weeklyMvpCount*WEEKLY_MVP_HALL_POINTS*10)/10;
  const total=Math.round((milestonePoints+awardPoints+weeklyMvpPoints)*10)/10;
  return {total,milestonePoints,awardPoints,weeklyMvpCount,weeklyMvpPoints,milestones,officialAwards,status:hallStatus(total)};
}
...
function formulaImpact(p:Player,roster:Player[]){
  ...
  return components.scoring*.25+components.rebounding*.25+components.playmaking*.25+components.defense*.20+components.ballSecurity*.05;
}
...
function exportData(data:LeagueData){
  const blob=new Blob([JSON.stringify({...data,exportedAt:new Date().toISOString(),version:"6.3.1"},null,2)],{type:"application/json"});
...
export default function App(){
  ...
  const [sundaySessions,setSundaySessions]=useState<SundaySession[]>(()=>loadData("yg-sunday-sessions",initialSundaySessions));
  const [statBaseline,setStatBaseline]=useState<PlayerStatBaseline>(()=>loadData("yg-stat-baseline",makeStatBaseline(loadData("yg-players",initialPlayers))));
  ...
  const saveAll=(next?:Partial<LeagueData>)=>{
    const p=next?.players??players,g=next?.games??games,a=next?.awards??awards,se=next?.seasons??seasons,n=next?.news??news,r=next?.runs??runs,po=next?.polls??polls,h=next?.history??history,b=next?.branding??branding,pr=next?.rankings??rankings,su=next?.submissions??submissions,ss=next?.sundaySessions??sundaySessions,sb=next?.statBaseline??statBaseline;
    localStorage.setItem("yg-players",JSON.stringify(p));localStorage.setItem("yg-games",JSON.stringify(g));localStorage.setItem("yg-awards",JSON.stringify(a));localStorage.setItem("yg-seasons",JSON.stringify(se));localStorage.setItem("yg-news",JSON.stringify(n));localStorage.setItem("yg-runs",JSON.stringify(r));localStorage.setItem("yg-polls",JSON.stringify(po));localStorage.setItem("yg-history",JSON.stringify(h));localStorage.setItem("yg-branding",JSON.stringify(b));localStorage.setItem("yg-rankings",JSON.stringify(pr));localStorage.setItem("yg-submissions",JSON.stringify(su));localStorage.setItem("yg-sunday-sessions",JSON.stringify(ss));localStorage.setItem("yg-stat-baseline",JSON.stringify(sb));
    const data={players:p,games:g,awards:a,seasons:se,news:n,runs:r,polls:po,history:h,branding:b,rankings:pr,submissions:su,sundaySessions:ss,statBaseline:sb};
    if(sessionToken){
  ...
  const resetData=()=>{
    ...
    replaceData({players:initialPlayers,games:initialGames,awards:initialAwards,seasons:initialSeasons,news:initialNews,runs:initialRuns,polls:initialPolls,history:initialHistory,branding:initialBranding,rankings:initialRankings,submissions:initialSubmissions,sundaySessions:initialSundaySessions,statBaseline:makeStatBaseline(initialPlayers)});
  };

  const filtered=useMemo(()=>players.filter(p => `${p.name} ${p.nickname}`.toLowerCase().includes(search.toLowerCase())),[players,search]);
  ...
  const updateOwnPhoto=async(playerId:string,photoUrl:string)=>{
    const response=await fetch("/api/player-profile",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({playerId,photoUrl})});
    if(!response.ok){const result=await response.json().catch(()=>({}));throw new Error(result.error||"Profile picture could not be saved");}
    const result=await response.json();setPlayers(result.players);localStorage.setItem("yg-players",JSON.stringify(result.players));
  ...
  const updateSundaySessions=(nextSessions:SundaySession[])=>{
    ...
    if(selected)setSelected(nextPlayers.find(player=>player.id===selected.id)??null);
    saveAll({sundaySessions:nextSessions,players:nextPlayers,statBaseline});
  };
  ...
  return <div className="app"><style>{styles}</style><style>{reminderStyles}</style>
    <header className="topbar">
      <button className="brand" onClick={()=>go("home")}><img className="brandMark" src={branding.logoUrl||initialBranding.logoUrl} alt=""/><span><b>{branding.wordmark}</b><small>{branding.tagline} · v6.3.1</small></span></button>
    ...
    <main>
      {view==="home" && <>
        ...
        {nextRun&&<QuickRsvp run={nextRun} myPlayer={myPlayer} onChoosePlayer={()=>setShowMyPlayerPicker(true)} onSubmit={submitRsvp}/>}
      ...
      {view==="community" && <Page eyebrow="THE COMMUNITY · v6.3.1" title="More than a box score." subtitle="Stories, announcements and moments from around the Y's Guys universe.">
      ...
      {view==="games" && <Page eyebrow="GAME DAY · v6.3.1" title="Schedule and results." subtitle="Upcoming matchups and complete results from the active season.">
      ...
      {view==="records" && <Page eyebrow="THE ARCHIVE" title="Y's Guys Record Book" subtitle="The marks everyone is chasing.">
        ...
        <div className="note">Version 6.3.1 recalculates this record book whenever shared player or game data changes.</div>
      </Page>}
      ...
      {view==="commissioner" && <Page eyebrow="COMMISSIONER MODE · v6.3.1" title="Run the entire league universe." subtitle="Publish games, profiles, photos and community stories for every visitor.">
        ...
        {adminTab==='sessions' && <SundaySessionManager sessions={sundaySessions} players={players} sessionToken={sessionToken} onChange={updateSundaySessions}/>}
        ...
        {adminTab==='polls' && <PollManager polls={polls} players={players} onChange={(next)=>{const nextPlayers=syncWeeklyMvpPollCredits(players,next);setPolls(next);setPlayers(nextPlayers);saveAll({polls:next,players:nextPlayers});}}/>}
...
function QuickRsvp({run,myPlayer,onChoosePlayer,onSubmit}:{run:SundayRun;myPlayer?:Player;onChoosePlayer:()=>void;onSubmit:(runId:string,rsvp:Omit<RunRsvp,"updatedAt">)=>Promise<void>}){
...
function HallHub({players,awards,seasons,records,updatedAt,onOpen}:{players:Player[];awards:Award[];seasons:typeof initialSeasons;records:RecordItem[];updatedAt:string|null;onOpen:(player:Player)=>void}){
  ...
  return <Page eyebrow="THE HALL · FORMULA V1" title="Records, banners and immortality." subtitle="The complete league archive in one place. Awards, milestones, wins and Weekly MVPs build Hall Progress.">
...
function PlayerDirectoryCard({player,roster,isMyPlayer,onOpen,onPhotoSave}:{player:Player;roster:Player[];isMyPlayer:boolean;onOpen:()=>void;onPhotoSave:(playerId:string,photoUrl:string)=>Promise<void>}){
  const [busy,setBusy]=useState(false);const [error,setError]=useState("");
  const choose=async(file?:File)=>{if(!file)return;setBusy(true);setError("");try{const imageUrl=await compressImage(file);await onPhotoSave(player.id,imageUrl)}catch(reason){setError(reason instanceof Error?reason.message:"Profile picture could not be saved")}finally{setBusy(false)}};
...
function CareerBestSection({sessions}:{sessions:{session:SundaySession;line:SundayStatLine}[]}){
  const categories=[["Points","pts"],["Rebounds","reb"],["Assists","ast"]] as const;
  const highs=categories.map(([label,key])=>{
    const max=sessions.reduce((best,item)=>Math.max(best,item.line[key]),0);
    const dates=[...new Set(sessions.filter(item=>item.line[key]===max&&max>0).map(item=>item.session.date))];
    return {label,max,dates};
  });
  return <section className="profilePanel careerBests">
    <Section eyebrow="CAREER BESTS" title="Single-day highs"/>
...
function PhotoSubmission({player,onSave}:{player:Player;onSave:(playerId:string,photoUrl:string)=>Promise<void>}){
  ...
  const send=async()=>{if(!imageUrl)return;setBusy(true);setError("");try{await onSave(player.id,imageUrl);setImageUrl("")}catch(error){setError(error instanceof Error?error.message:"Profile picture could not be saved")}finally{setBusy(false)}};
  return <section className="profilePanel photoSubmit"><Section eyebrow="MY PLAYER" title="Update profile picture"/><p>Preview your picture, then save it. It updates immediately across the league.</p>{imageUrl&&<img src={imageUrl} alt="Photo preview"/>}<label className="uploadButton">Choose picture<input type="file" accept="image/*" onChange={event=>choose(event.target.files?.[0])}/></label><button className="primary" disabled={!imageUrl||busy} onClick={send}>{busy?"Saving…":"Save profile picture"}</button>{error&&<p className="formError">{error}</p>}</section>;
}
...
function SuggestionBox({players,defaultPlayerId,onSubmit}:{players:Player[];defaultPlayerId:string;onSubmit:(item:{type:"profile-photo"|"suggestion";playerId:string;message:string;imageUrl?:string})=>Promise<void>}){
  ...
  return <Page eyebrow="PRIVATE INBOX · v6.3.1" title="Suggestion Box" subtitle="Send an idea, correction or picture directly to the Commissioner. Nothing appears publicly unless the Commissioner chooses to use it."><section className="suggestionCard">{done&&<div className="successNote">✓ Your suggestion is in the Commissioner Review Center.</div>}<label>Your player profile<select value={playerId} onChange={event=>setPlayerId(event.target.value)}>{players.map(player=><option value={player.id} key={player.id}>{player.name}</option>)}</select></label><label>Suggestion, idea or correction<textarea value={message} onChange={event=>setMessage(event.target.value)} maxLength={1200} placeholder="What should be added, corrected or remembered?"/></label><label className="uploadButton">Add one optional picture<input type="file" accept="image/*" onChange={event=>choose(event.target.files?.[0])}/></label>{imageUrl&&<div className="suggestionPreview"><img src={imageUrl} alt="Suggestion preview"/><button onClick={()=>setImageUrl("")}>Remove</button></div>}<p className="privacyNote">Pictures are compressed before upload. Video is not accepted in Version 6.3.1.</p><button className="primary" disabled={busy} onClick={send}>{busy?"Sending…":"Send privately"}</button>{error&&<p className="formError">{error}</p>}</section></Page>;
}
...
type RankingPasteRow={player:Player;dnp:boolean;reason:string};
type RankingPastePreview={rows:RankingPasteRow[];missing:Player[];errors:string[]};
function rankingNameKey(value:string){return value.toLowerCase().replace(/[’']/g,"").replace(/[^a-z0-9]+/g," ").trim()}
function rankingPlayerAliases(players:Player[]){
  const aliases=new Map<string,Player[]>();
  players.forEach(player=>[player.name,player.nickname].filter(Boolean).forEach(value=>{const key=rankingNameKey(value);if(!key)return;const matches=aliases.get(key)??[];if(!matches.some(match=>match.id===player.id))aliases.set(key,[...matches,player])}));
  return aliases;
}
function rankingDraft(latest:PowerRankingSnapshot,players:Player[]){
  const aliases=rankingPlayerAliases(players),used=new Set<string>(),draft:RankingEntry[]=[];
  latest.entries.forEach(entry=>{const matches=entry.playerId?players.filter(player=>player.id===entry.playerId):(aliases.get(rankingNameKey(entry.playerName))??[]);const player=matches.length===1?matches[0]:undefined;if(!player||used.has(player.id))return;used.add(player.id);draft.push({...entry,playerId:player.id,playerName:player.name})});
  players.forEach(player=>{if(!used.has(player.id))draft.push({playerId:player.id,playerName:player.name,rank:null,movement:0,dnp:true,reason:"DNP"})});
  return draft;
}
function parseRankingPaste(text:string,players:Player[]):RankingPastePreview{
  const aliases=rankingPlayerAliases(players),rows:RankingPasteRow[]=[],errors:string[]=[];let dnpSection=false;
  text.split(/\r?\n/).forEach((raw,lineIndex)=>{
    let line=raw.trim();if(!line)return;
    if(/^\s*(?:power\s+)?rankings?(?:\s+week\s+\d+)?\s*:?\s*$/i.test(line)||/^week\s+\d+\s*:?\s*$/i.test(line)){dnpSection=false;return}
    if(/^dnp(?:\s+players?)?\s*:?\s*$/i.test(line)){dnpSection=true;return}
    line=line.replace(/^[•*]\s*/,"").replace(/^[-–—]\s+/,"").replace(/^#?\d+\s*(?:[.)\-:]\s*|\s+)/,"").trim();
    const dnp=dnpSection||/^dnp\s*[:\-]\s*/i.test(line)||/\s*[([]?dnp[)\]]?\s*$/i.test(line);
    line=line.replace(/^dnp\s*[:\-]\s*/i,"").replace(/\s*[([]?dnp[)\]]?\s*$/i,"").trim();
    let name=line,reason="";const separator=line.match(/\s+(?:\||—|–|-)\s+/);
    if(separator?.index!==undefined){name=line.slice(0,separator.index).trim();reason=line.slice(separator.index+separator[0].length).trim()}
    else if(line.includes("|")){const split=line.indexOf("|");name=line.slice(0,split).trim();reason=line.slice(split+1).trim()}
    const matches=aliases.get(rankingNameKey(name))??[];
    if(matches.length!==1){errors.push(matches.length>1?`Line ${lineIndex+1}: “${name}” matches more than one player.`:`Line ${lineIndex+1}: “${name||raw.trim()}” was not recognized.`);return}
    if(rows.some(row=>row.player.id===matches[0].id)){errors.push(`Line ${lineIndex+1}: ${matches[0].name} is listed more than once.`);return}
    rows.push({player:matches[0],dnp,reason:dnp?"DNP":reason});
  });
  const used=new Set(rows.map(row=>row.player.id));
  return {rows,missing:players.filter(player=>!used.has(player.id)),errors};
}
...
function PowerRankingManager({rankings,players,onChange}:{rankings:PowerRankingSnapshot[];players:Player[];onChange:(next:PowerRankingSnapshot[])=>void}){
  const latest=rankings[0]??initialRankings[0];const [week,setWeek]=useState(latest.week+1);const [date,setDate]=useState(new Date().toLocaleDateString("en-CA"));const [entries,setEntries]=useState<RankingEntry[]>(()=>rankingDraft(latest,players));const [pasteText,setPasteText]=useState("");const [pasteNote,setPasteNote]=useState("");
  const pastePreview=useMemo(()=>parseRankingPaste(pasteText,players),[pasteText,players]);
  const rankedPreview=pastePreview.rows.filter(row=>!row.dnp),explicitDnp=pastePreview.rows.filter(row=>row.dnp),dnpPreview=[...explicitDnp.map(row=>row.player),...pastePreview.missing];
  ...
  const loadCurrent=()=>{let rank=0;setPasteText(entries.map(entry=>entry.dnp?`DNP: ${entry.playerName}`:`${++rank}. ${entry.playerName}${entry.reason&&entry.reason!=="Commissioner ranking published."?` | ${entry.reason}`:""}`).join("\n"));setPasteNote("Current order loaded. Edit it, then apply the pasted order.")};
  const copyRoster=async()=>{const roster=players.map(player=>player.name).join("\n");try{await navigator.clipboard.writeText(roster);setPasteNote("Player list copied.")}catch{setPasteText(roster);setPasteNote("Player list loaded in the box so you can copy it.")}};
  const applyPaste=()=>{if(!pastePreview.rows.length||pastePreview.errors.length)return;const ranked=rankedPreview.map((row,index)=>({playerId:row.player.id,playerName:row.player.name,rank:index+1,movement:0,dnp:false,reason:row.reason}));const dnp=dnpPreview.map(player=>({playerId:player.id,playerName:player.name,rank:null,movement:0,dnp:true,reason:"DNP"}));setEntries([...ranked,...dnp]);setPasteNote(`Applied ${ranked.length} ranked players · ${dnp.length} marked DNP.`)};
  const publish=()=>{const previous=new Map(latest.entries.filter(item=>item.playerId&&item.rank).map(item=>[item.playerId!,item.rank!]));let nextRank=0;const published:PowerRankingSnapshot={id:makeId("power"),week,date,publishedAt:new Date().toISOString(),entries:entries.map(entry=>{const dnp=entry.dnp;const rank=dnp?null:++nextRank;const old=entry.playerId?previous.get(entry.playerId):undefined;return {...entry,rank,movement:dnp?0:old?old-(rank??old):0,reason:dnp?"DNP":entry.reason||"Commissioner ranking published."}})};onChange([published,...rankings]);setWeek(value=>value+1)};
  let visibleRank=0;
  return <section className="adminCard"><Section eyebrow="WEEKLY PUBLISHER" title="Power Rankings"/><div className="formGrid"><label>Week<input type="number" value={week} onChange={event=>setWeek(Number(event.target.value))}/></label><label>Ranking date<input type="date" value={date} onChange={event=>setDate(event.target.value)}/></label></div><section className="rankingPaste"><div><span>FAST ENTRY</span><h3>Paste the full ranking order</h3><p>Use one player per line. Numbered lists work. Add <b>| a reason</b> after a name if desired; anyone omitted is previewed as DNP.</p></div><textarea value={pasteText} onChange={event=>{setPasteText(event.target.value);setPasteNote("")}} placeholder={"1. Steve | Strong all-around week\n2. Paul Peters\n3. Vic\nDNP: Mike"}/><div className="rankingPasteActions"><button className="secondary" onClick={copyRoster}>Copy player list</button><button className="secondary" onClick={loadCurrent}>Load current order</button><button className="primary" disabled={!pastePreview.rows.length||Boolean(pastePreview.errors.length)} onClick={applyPaste}>Apply pasted order</button></div>{pasteText&&<div className={`rankingPastePreview ${pastePreview.errors.length?"hasErrors":""}`}><strong>{pastePreview.errors.length?"Fix before applying":`${rankedPreview.length} ranked · ${dnpPreview.length} DNP`}</strong>{pastePreview.errors.length?<ul>{pastePreview.errors.map(error=><li key={error}>{error}</li>)}</ul>:<><div className="rankingPasteList">{rankedPreview.map((row,index)=><span key={row.player.id}><b>#{index+1}</b> {row.player.name}</span>)}</div>{dnpPreview.length>0&&<small>DNP: {dnpPreview.map(player=>player.name).join(", ")}</small>}</>}</div>}{pasteNote&&<p className="rankingPasteNote">✓ {pasteNote}</p>}</section><div className="rankingEditor">{entries.map((entry,index)=>{const rank=entry.dnp?null:++visibleRank;return <article key={entry.playerId}><strong>{entry.dnp?"DNP":rank}</strong><span><b>{entry.playerName}</b><textarea value={entry.reason} onChange={event=>patch(index,{reason:event.target.value})} placeholder="Why this ranking?"/></span><label><input type="checkbox" checked={entry.dnp} onChange={event=>patch(index,{dnp:event.target.checked})}/> DNP</label><div><button disabled={index===0} onClick={()=>move(index,-1)} aria-label={`Move ${entry.playerName} up`}>↑</button><button disabled={index===entries.length-1} onClick={()=>move(index,1)} aria-label={`Move ${entry.playerName} down`}>↓</button></div></article>})}</div><button className="primary" onClick={publish}>Publish Week {week} rankings</button></section>;
}
...
function LeagueTimeline({games,awards,news,runs,history}:{games:Game[];awards:Award[];news:NewsStory[];runs:SundayRun[];history:HistoryEntry[]}){
  ...
  return <Page eyebrow="LEAGUE HISTORY · v6.3.1" title="The Y's Guys timeline." subtitle="Games, awards, stories, Sundays and Commissioner-recorded memories preserved together."><div className="chips">{["All","History","Games","Awards","News","Sundays"].map(value=><button className={filter===value?"active":""} onClick={()=>setFilter(value)} key={value}>{value}</button>)}</div><section className="timeline">{visible.map(item=><article className={`timelineItem ${item.imageUrl?"withPhoto":""}`} key={item.id}>{item.imageUrl&&<img src={item.imageUrl} alt=""/>}<span className="timelineIcon">{item.icon}</span><div><small>{item.kind} · {item.date}</small><h2>{item.title}</h2><p>{item.copy}</p></div></article>)}</section></Page>;
}
...
function AttendanceCenter({runs,players,defaultPlayerId,onSubmit}:{runs:SundayRun[];players:Player[];defaultPlayerId:string;onSubmit:(runId:string,rsvp:Omit<RunRsvp,"updatedAt">)=>Promise<void>}){
  ...
  return <Page eyebrow="SUNDAY RUN · v6.3.1" title="Are you in?" subtitle="Choose your name and let the group know your Sunday availability. No account or PIN required.">
...
type ReminderStatus = "checking" | "ready" | "enabled" | "denied" | "unsupported";
...
function FridayReminderCard(){
  ...
  useEffect(()=>{
    ...
    const inspect=async()=>{
      ...
      if(!window.isSecureContext||!("serviceWorker" in navigator)||!("PushManager" in window)||!("Notification" in window)||(ios&&!standalone)){
  ...
  const disable=async()=>{
    ...
    try{
      ...
      if(subscription){
        ...
        const result=await response.json().catch(()=>({}));
        if(!response.ok)throw new Error(result.error||"The reminder could not be disabled.");
  ...
  const ios=/iPhone|iPad|iPod/i.test(navigator.userAgent);
  const copy=status==="enabled"
    ?"You’ll get a notification every Friday at 6:00 PM to mark your Sunday availability."
    :status==="unsupported"&&ios
      ?"On iPhone, add Y’s Guys to your Home Screen first. Open that app icon, return here, then enable notifications."
      :status==="unsupported"
        ?"This browser does not support app notifications. Try the installed Y’s Guys app in a current browser."
        :status==="denied"
          ?"Notifications are blocked. Allow Y’s Guys in your device notification settings, then return here."
...
function RuleBook(){
  ...
  return <Page eyebrow="OFFICIAL RULE BOOK · v6.3.1" title="How the Y's Guys universe operates." subtitle="The official rules of the Sunday run."><div className="ruleHero"><span>YG</span><div><b>LEAGUE STANDARD</b><h2>Have fun win the day!</h2></div></div><div className="rulesGrid">{sections.map((section,index)=><section className="ruleSection" key={section.title}><div><span>{section.icon}</span><small>ARTICLE {index+1}</small></div><h2>{section.title}</h2><ol start={index+1}>{section.rules.map(rule=><li key={rule}>{rule}</li>)}</ol></section>)}</div></Page>;
}
...
type SundayDraftLine=SundayStatLine&{included:boolean;uncertain?:boolean};
function SundaySessionManager({sessions,players,sessionToken,onChange}:{sessions:SundaySession[];players:Player[];sessionToken:string;onChange:(sessions:SundaySession[])=>void}){
  ...
  const [lines,setLines]=useState<SundayDraftLine[]>(blankLines);
  const [scanning,setScanning]=useState(false);
  const [scanMessage,setScanMessage]=useState("");
  useEffect(()=>setLines(current=>players.map(player=>current.find(line=>line.playerId===player.id)??{playerId:player.id,gp:0,wins:0,pts:0,reb:0,ast:0,turnovers:0,included:false})),[players]);
  const reset=()=>{setEditingId("");setDate(new Date().toLocaleDateString("en-CA"));setNotes("");setSourceImageUrl("");setLines(blankLines());setScanMessage("")};
  const update=(playerId:string,key:keyof SundayStatLine,value:number)=>setLines(current=>current.map(line=>line.playerId===playerId?{...line,[key]:Math.max(0,Math.round(value)),included:true,uncertain:false}:line));
  const validation=useMemo(()=>{
    const included=lines.filter(line=>line.included);
    const errors:string[]=[];
    if(!date)errors.push("Choose the Sunday date.");
    if(!included.length)errors.push("Include at least one player.");
    included.forEach(line=>{const name=players.find(player=>player.id===line.playerId)?.name??"Player";if(line.gp<1)errors.push(`${name}: games played must be at least 1.`);if(line.wins>line.gp)errors.push(`${name}: wins cannot exceed games played.`);if([line.pts,line.reb,line.ast,line.turnovers].some(value=>value>500))errors.push(`${name}: unusually large total needs review.`)});
    return errors;
  },[date,lines,players]);
  const previewRoster=useMemo(()=>{
    const prior=sessions.find(session=>session.id===editingId);
    return players.map(player=>{
      const oldLine=prior?.lines.find(line=>line.playerId===player.id);
      const newLine=lines.find(line=>line.playerId===player.id&&line.included);
      return {...player,
        wins:Math.max(0,player.wins-(oldLine?.wins??0)+(newLine?.wins??0)),
        losses:Math.max(0,player.losses-(oldLine?oldLine.gp-oldLine.wins:0)+(newLine?newLine.gp-newLine.wins:0)),
        pts:Math.max(0,player.pts-(oldLine?.pts??0)+(newLine?.pts??0)),
        reb:Math.max(0,player.reb-(oldLine?.reb??0)+(newLine?.reb??0)),
        ast:Math.max(0,player.ast-(oldLine?.ast??0)+(newLine?.ast??0)),
        turnovers:Math.max(0,player.turnovers-(oldLine?.turnovers??0)+(newLine?.turnovers??0)),
      };
    });
  },[editingId,lines,players,sessions]);
  const chooseImage=async(file?:File)=>{if(!file)return;setSourceImageUrl(await compressImage(file));setScanMessage("Picture attached. Scan it, then verify every total before publishing.")};
  const scan=async()=>{
    if(!sourceImageUrl)return;setScanning(true);setScanMessage("");
    try{
      const response=await fetch("/api/scan-stats",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${sessionToken}`},body:JSON.stringify({imageDataUrl:sourceImageUrl,players:players.map(player=>({id:player.id,name:player.name,nickname:player.nickname}))})});
      const result=await response.json();if(!response.ok)throw new Error(result.error||"Picture could not be scanned");
      const extracted=Array.isArray(result.lines)?result.lines:[];
      setLines(players.map(player=>{const found=extracted.find((line:SundayDraftLine)=>line.playerId===player.id);return found?{...found,included:true,uncertain:Boolean(found.uncertain)}:{playerId:player.id,gp:0,wins:0,pts:0,reb:0,ast:0,turnovers:0,included:false}}));
      setScanMessage(result.warning||"Scan complete. Highlighted rows need extra attention; verify every value.");
    }catch(reason){setScanMessage(reason instanceof Error?reason.message:"Picture could not be scanned")}finally{setScanning(false)}
  };
  const publish=()=>{
    if(validation.length)return;
    const now=new Date().toISOString(),existing=sessions.find(session=>session.id===editingId);
    const session:SundaySession={id:editingId||makeId("session"),date,notes:notes.trim(),status:"published",lines:lines.filter(line=>line.included).map(({included:_,uncertain:__,...line})=>line),sourceImageUrl:sourceImageUrl||undefined,createdAt:existing?.createdAt??now,updatedAt:now};
    const next=existing?sessions.map(item=>item.id===existing.id?session:item):[...sessions,session];
    onChange(next);reset();
  };
  const edit=(session:SundaySession)=>{setEditingId(session.id);setDate(session.date);setNotes(session.notes);setSourceImageUrl(session.sourceImageUrl??"");setLines(players.map(player=>{const line=session.lines.find(item=>item.playerId===player.id);return line?{...line,included:true}:{playerId:player.id,gp:0,wins:0,pts:0,reb:0,ast:0,turnovers:0,included:false}}));window.scrollTo({top:0,behavior:"smooth"})};
  const remove=(session:SundaySession)=>{if(confirm(`Delete the ${formatRunDate(session.date)} stats? Career totals will be recalculated. The cloud revision remains available in Backups.`))onChange(sessions.filter(item=>item.id!==session.id))};
  return <div className="sessionManager">
    <section className="adminCard sessionEntry"><div className="sessionEntryHead"><div><span>VERSION 6 ENTRY SYSTEM</span><h2>{editingId?"Correct Sunday Session":"New Sunday Session"}</h2><p>Enter the entire day once. Nothing changes until you publish.</p></div><label>Sunday date<input type="date" value={date} onChange={event=>setDate(event.target.value)}/></label></div>
      <div className="scanBox"><label className="uploadButton">{sourceImageUrl?"Replace stats picture":"Upload weekly stats picture"}<input type="file" accept="image/*" onChange={event=>chooseImage(event.target.files?.[0])}/></label>{sourceImageUrl&&<><img src={sourceImageUrl} alt="Weekly totals source"/><button type="button" className="secondary" disabled={scanning} onClick={scan}>{scanning?"Reading picture…":"Scan picture with AI"}</button></>}{scanMessage&&<p>{scanMessage}</p>}</div>
      <div className="weeklyStatsTable"><div className="weeklyStatsHead"><b>Use</b><b>Player</b><b>GP</b><b>W</b><b>PTS</b><b>REB</b><b>AST</b><b>TO</b></div>{lines.map(line=>{const player=players.find(item=>item.id===line.playerId)!;return <div className={line.uncertain?"uncertain":""} key={line.playerId}><input aria-label={`Include ${player.name}`} type="checkbox" checked={line.included} onChange={event=>setLines(current=>current.map(item=>item.playerId===line.playerId?{...item,included:event.target.checked}:item))}/><b>{player.name}{line.uncertain?<small>CHECK</small>:null}</b>{(["gp","wins","pts","reb","ast","turnovers"] as const).map(key=><input aria-label={`${player.name} ${key}`} type="number" min="0" inputMode="numeric" value={line[key]} onChange={event=>update(line.playerId,key,Number(event.target.value))} key={key}/>)}</div>})}</div>
      <label className="sessionNotes">Commissioner notes<textarea value={notes} onChange={event=>setNotes(event.target.value)} placeholder="Optional context, corrections, or source notes…"/></label>
      <section className="publishPreview"><h3>Publish preview</h3><p>{lines.filter(line=>line.included).length} players · {lines.filter(line=>line.included).reduce((sum,line)=>sum+line.gp,0)} player-games · career totals, Overall, highs, records, banners and Hall progress will recalculate together.</p>{lines.filter(line=>line.included||sessions.find(session=>session.id===editingId)?.lines.some(old=>old.playerId===line.playerId)).map(line=>{const before=players.find(player=>player.id===line.playerId)!,after=previewRoster.find(player=>player.id===line.playerId)!;return <div className="playerChangePreview" key={line.playerId}><b>{before.name}</b><span>{before.wins}-{before.losses} → <strong>{after.wins}-{after.losses}</strong></span><span>PTS {before.pts} → <strong>{after.pts}</strong></span><span>REB {before.reb} → <strong>{after.reb}</strong></span><span>AST {before.ast} → <strong>{after.ast}</strong></span><span>TO {before.turnovers} → <strong>{after.turnovers}</strong></span><span>OVR {overallRating(before,players)??"PROV"} → <strong>{overallRating(after,previewRoster)??"PROV"}</strong></span></div>})}{validation.map(error=><div className="formError" key={error}>{error}</div>)}</section>
      <div className="formActions"><button type="button" className="primary" disabled={validation.length>0} onClick={publish}>{editingId?"Publish correction":"Publish Sunday once"}</button>{editingId&&<button type="button" className="secondary" onClick={reset}>Cancel correction</button>}</div>
    </section>
    <ManageList title="Sunday Session history" empty="No weekly stats have been published yet.">{[...sessions].sort((a,b)=>b.date.localeCompare(a.date)).map(session=><div className="manageRow sessionHistoryRow" key={session.id}><div><b>{formatRunDate(session.date)}</b><small>{session.lines.length} players · {session.lines.reduce((sum,line)=>sum+line.gp,0)} player-games · edited {new Date(session.updatedAt).toLocaleString()}</small></div>{session.sourceImageUrl&&<span title="Source picture retained">📷</span>}<button onClick={()=>edit(session)}>Review / Correct</button><button className="deleteLink" onClick={()=>remove(session)}>Delete</button></div>)}</ManageList>
  </div>;
}
...
function PollManager({polls,players,onChange}:{polls:LeaguePoll[];players:Player[];onChange:(polls:LeaguePoll[])=>void}){
  ...
  const editing=Boolean(draft.id);
  const save=(event:React.FormEvent)=>{event.preventDefault();if(!draft.title.trim()||draft.nomineeIds.length<2)return alert("Add a title and at least two nominees.");if(draft.category==="Weekly Award"&&draft.status==="closed"&&!draft.officialWinnerId)return alert("Choose the official Weekly MVP before closing this poll.");const clean={...draft,id:draft.id||makeId("poll"),title:draft.title.trim(),description:draft.description.trim(),finalizedAt:draft.status==="closed"&&draft.officialWinnerId?(draft.finalizedAt||new Date().toISOString()):undefined};onChange(editing?polls.map(poll=>poll.id===clean.id?clean:poll):[clean,...polls]);setDraft(empty)};
...
.reminderCard{margin-top:18px;display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:16px;align-items:center;padding:20px 22px;border:1px solid #dfc77f;border-radius:22px;background:linear-gradient(135deg,#fff9e9,#fff);box-shadow:0 12px 28px rgba(65,48,12,.08)}
.reminderCard.enabled{border-color:#9ac8aa;background:linear-gradient(135deg,#edf9f1,#fff)}
.reminderIcon{width:48px;height:48px;display:grid;place-items:center;border-radius:15px;background:#0A2D5E;font-size:22px}
.reminderCard small{font-size:9px;letter-spacing:.13em;color:#967224;font-weight:1000}
.reminderCard h2{margin:4px 0;font-size:20px}
...
.sessionManager{display:grid;gap:18px}.sessionEntryHead{display:flex;justify-content:space-between;gap:20px;align-items:start}.sessionEntryHead span{font-size:10px;letter-spacing:.14em;color:#987323;font-weight:1000}.sessionEntryHead h2{margin:5px 0}.sessionEntryHead p{color:#718096;margin:0}.sessionEntryHead label,.sessionNotes{display:flex;flex-direction:column;gap:7px;font-size:11px;font-weight:900}.sessionEntryHead input,.sessionNotes textarea{border:1px solid #dbe1e9;border-radius:11px;padding:11px;font:inherit}.scanBox{margin:20px 0;padding:16px;border:1px dashed #cfb56f;border-radius:16px;background:#fffaf0;display:flex;align-items:center;gap:12px;flex-wrap:wrap}.scanBox img{width:90px;height:90px;object-fit:cover;border-radius:12px}.scanBox p{width:100%;margin:0;color:#665d4a;font-size:12px}.weeklyStatsTable{border:1px solid #dfe4eb;border-radius:16px;overflow:auto}.weeklyStatsTable>div{min-width:760px;display:grid;grid-template-columns:48px minmax(140px,1fr) repeat(6,70px);align-items:center;gap:7px;padding:9px 12px;border-top:1px solid #edf0f4}.weeklyStatsTable>div:first-child{border-top:0}.weeklyStatsHead{background:${NAVY};color:white;font-size:10px;letter-spacing:.06em}.weeklyStatsTable input[type=number]{width:100%;padding:8px 5px;border:1px solid #dbe1e9;border-radius:8px;text-align:center}.weeklyStatsTable .uncertain{background:#fff1cc}.weeklyStatsTable b{display:flex;align-items:center;gap:7px}.weeklyStatsTable b small{color:#9a671a;background:#ffe3a1;border-radius:999px;padding:3px 5px;font-size:8px}.sessionNotes{margin-top:16px}.sessionNotes textarea{min-height:80px}.publishPreview{margin-top:16px;padding:15px;border-radius:15px;background:#eef4fa}.publishPreview h3{margin:0 0 5px}.publishPreview>p{margin:0 0 12px;color:#5d6e84}.playerChangePreview{display:grid;grid-template-columns:minmax(100px,1fr) repeat(6,auto);gap:10px;padding:9px 0;border-top:1px solid #d9e2ec;font-size:11px}.playerChangePreview span{color:#68788c}.playerChangePreview strong{color:${NAVY}}.sessionHistoryRow{grid-template-columns:minmax(0,1fr) auto auto auto}.myPlayerPerformance,.careerBests{margin-top:18px}.careerHighGrid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.careerHighGrid article{padding:17px;border:1px solid #dec98a;border-radius:18px;background:linear-gradient(145deg,#fffdf6,#f7e9bd);display:flex;flex-direction:column}.careerHighGrid small{font-size:8px;letter-spacing:.1em;color:#967224;font-weight:1000}.careerHighGrid strong{font-size:36px;margin:5px 0}.careerHighGrid b{font-size:12px}.careerHighGrid span{font-size:9px;color:#718096;margin-top:7px}.sundayLog{background:white;border:1px solid #e1e6ed;border-radius:18px;overflow:auto}.sundayLog>div{min-width:650px;display:grid;grid-template-columns:100px 45px 70px repeat(4,55px);gap:8px;align-items:center;text-align:center;padding:12px 15px;border-top:1px solid #edf0f4}.sundayLog>div:first-child{border-top:0}.sundayLogHead{background:${NAVY};color:white;font-size:10px}.sundayLog>div>b{text-align:left}
@media(max-width:900px){.careerHighGrid{grid-template-columns:repeat(3,1fr)}.playerChangePreview{grid-template-columns:1fr repeat(3,auto)}.playerChangePreview span:nth-of-type(n+4){display:none}}@media(max-width:640px){.sessionEntryHead{flex-direction:column}.sessionEntryHead label{width:100%}.sessionEntryHead input{width:100%}.careerHighGrid{grid-template-columns:repeat(2,1fr)}.sessionHistoryRow{grid-template-columns:1fr auto}.sessionHistoryRow>div{grid-column:1/-1}.rankingPaste{padding:14px}.rankingPasteActions{display:grid;grid-template-columns:1fr 1fr}.rankingPasteActions .primary{grid-column:1/-1}.rankingPasteList{grid-template-columns:repeat(2,minmax(0,1fr))}.rankingEditor article{grid-template-columns:36px minmax(0,1fr) auto}.rankingEditor article>div{grid-column:2/-1;justify-content:flex-end}.rankingEditor article>div button{min-width:44px}}
`;
