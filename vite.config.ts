...
import { version69HallHistory } from "./version69hallhistory.plugin";
import { version691RecordRules } from "./version691recordrules.plugin";
...
import { version7LayoutFix } from "./version7layoutfix.plugin";
import { version71CombinedDefense } from "./version71.plugin";
import { version711MyPlayerFix } from "./version711myplayerfix.plugin";
...
function closeSundayFeature(): Plugin {
  return {name:"ys-guys-close-sunday",enforce:"pre",transform(source,id){
    ...
    code=replaceRequired(code,'  const generateSundays=()=>{',`  const closeRun=(run:SundayRun)=>{if(!confirm(\`Close \${formatRunDate(run.date)}? Attendance and recorded data will be preserved, and the next Sunday will become active.\`))return;const nextDate=localDate(run.date);nextDate.setDate(nextDate.getDate()+7);const nextDateKey=nextDate.toLocaleDateString("en-CA");const existingNext=runs.find(item=>item.date===nextDateKey);const updated=runs.map(item=>item.id===run.id?{...item,status:"closed" as const}:existingNext&&item.id===existingNext.id?{...item,status:"open" as const}:item);const nextRun:SundayRun|undefined=existingNext?undefined:{...run,id:makeId("run"),date:nextDateKey,status:"open",rsvps:[],deadline:""};onChange([...(updated),...(nextRun?[nextRun]:[])].sort((a,b)=>a.date.localeCompare(b.date)));setDraft(empty);};\n  const generateSundays=()=>{`);
